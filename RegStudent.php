...
<html lang="en">
  ...
  <body>
	...
	<div class="container">
		<form action="RegStudent.php" method="post" id="form" onsubmit="return validateForm()" class="width1">
			
			<legend><h4 class="display-5 text-center bg-light text-dark mb-3">REGISTRATION FORM</h4></legend>
		...
		<script>
			...
			<?php
				if(isset($_POST['name']) && isset($_POST['usn']) && isset($_POST['phno']) && isset($_POST['email']) && isset($_POST['password']) && isset($_POST['confirmpassword']) ){
					$name=$_POST['name'];
					$usn=$_POST['usn'];
					$phno=$_POST['phno'];
					$email=$_POST['email'];
					...
					if($notable=="1"){
						$db="Create database ELearning";	
						if(!(mysqli_query($con,$db))){
							echo"<script>";
							echo"alert('Could not create database')";
							echo"</script>";
						...
						fclose($file);
						$file=fopen("regstudent.txt","w");
						fwrite($file,"0");
						fclose($file);
						$insert="Insert into Students(NAME,USN,PHONE,EMAIL,PASSWORD) Values('$name','$usn','$phno','$email','$password');";
